...
Global
*/
// error Dlr_ApproveFail();
error Dlr_CalcFail(uint256, uint256);
error Dlr_ConverterFail(uint256);
...
library Global {
    ...
    function toUint128(uint256 value) internal pure returns (uint128) {
        if (value > type(uint128).max) {
            revert Dlr_ConverterFail(value);
        }
    ...
    function tryAdd(
        ...
        unchecked {
            uint256 c = a + b;
            if (c < a) revert Dlr_CalcFail(a, b);
            result = c;
        }
    }
    ...
    function trySub(
        ...
        unchecked {
            if (b > a) revert Dlr_CalcFail(a, b);
            result = a - b;
        }
    }
    ...
    function tryMul(
        ...
        unchecked {
            ...
            if (a == 0) result = 0;
            uint256 c = a * b;
            if (c / a != b) revert Dlr_CalcFail(a, b);
            result = c;
        }
    }
    ...
    function tryDiv(
        ...
        unchecked {
            if (b == 0) revert Dlr_CalcFail(a, b);
            result = a / b;
        }
    }
...
//     uint256 _value
// ) internal {
//     bool success = IERC20(_tokenAddress).approve(_to, _value);
//     if (!success) {
//         revert Dlr_ApproveFail();
//     }
// }
